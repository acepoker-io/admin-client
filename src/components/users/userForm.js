...
const UserForm = ({ userDetail, getAllUser, setShowInfo }) => {
  const {
    ...
    formState: { errors },
    setValue,
  } = useForm({ resolver: yupResolver(updateSchema),defaultValues: {
    ...
    <>
      <div className="opentableModal">
        ...
        <Form onSubmit={handleSubmit(updateUserDetail)}>
          ...
          <FormGroup>
            ...
            className="phoneInputUserInfo"
             ...
             value={phone || userDetail?.phone}
                  onChange={(value) => { setPhone(value); setValue("phone", `+${value}`) }}
              inputProps={{
                name: 'phone',
              }}
            />
          ...
          <Button className="loginPageBtn userInfoPopupBtn" color="primary" type="submit" >
